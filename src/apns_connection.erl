...
-export([start_link/2
    ...
    , push_notification/4
    , push_notification/5
    , bulk_push_notification/4
    , async_push_notification/4
]).
...
%% @doc Bulk pushes notification to certificate APNs connection.
-spec bulk_push_notification(name() | pid()
    , list()
    , notification()
    , apns:headers()) -> {ok, BulkTaskId::integer()}.
bulk_push_notification(ConnectionId, DeviceIdList, Notification, Headers) ->
    gen_server:call(ConnectionId, {bulk_push_notification, DeviceIdList, Notification, Headers}).


%% @doc Async pushes notification to certificate APNs connection.
-spec async_push_notification(name() | pid()
    , apns:device_id()
    , notification()
    , apns:headers()) -> {ok, BulkTaskId::integer()}.
async_push_notification(ConnectionId, DeviceId, Notification, Headers) ->
    gen_server:call(ConnectionId, {async_push_notification, DeviceId, Notification, Headers}).
...
handle_call({async_push_notification, DeviceId, Notification, Headers}
    , {From, _}
    , #{client := From} = State) ->
    #{connection := Connection, http2_connection := HTTP2Conn} = State,
    #{timeout := Timeout} = Connection,
    case push(HTTP2Conn, DeviceId, Headers, Notification, Connection) of
        {ok, StreamId} ->
            {reply, {ok, StreamId, Timeout}, State};
        {error, Err} ->
            {reply, {error, Err}, State}
    end;
handle_call({bulk_push_notification, DeviceIdList, Notification, Headers}
    , {From, _}
    , #{client := From} = State) ->
    #{connection := Connection, http2_connection := HTTP2Conn} = State,
%%    #{timeout := Timeout} = Connection,
    BulkTaskId = now_in_microseconds(),
    [
        gen_server:cast(
            self(),
            {bulk_push_notification, BulkTaskId, From, DeviceId, Notification, Headers})
        || DeviceId <- DeviceIdList
    ],
    {reply, {ok, BulkTaskId}, State};
handle_call({push_notification, DeviceId, Notification, Headers}
    , {From, _}
    , #{client := From} = State) ->
    #{connection := Connection, http2_connection := HTTP2Conn} = State,
    #{timeout := Timeout} = Connection,
    {ok, StreamId} = push(HTTP2Conn, DeviceId, Headers, Notification, Connection),
...
-spec handle_cast(Request :: term(), State) ->
    {noreply, State}.
handle_cast({bulk_push_notification, BulkTaskId, From, DeviceId, Notification, Headers}
    , #{client := From} = State) ->
    #{connection := Connection, http2_connection := HTTP2Conn} = State,
    #{timeout := Timeout} = Connection,
    case push(HTTP2Conn, DeviceId, Headers, Notification, Connection) of
        {ok, StreamId} ->
            From ! {push_success, BulkTaskId, StreamId, DeviceId};
        {error, Err} ->
            From ! {push_fail, BulkTaskId, DeviceId, Err}
    end,
    {noreply, State};
handle_cast({bulk_push_notification, BulkTaskId, From, DeviceId, _, _}, #{client := Client} = State) ->
    Client ! {push_fail, BulkTaskId, DeviceId, not_connection_owner},
    {noreply, State};
...
push(HTTP2Conn, DeviceId, HeadersMap, Notification, Connection) ->
    ...
    h2_client:send_request(HTTP2Conn, Headers, Notification).
